...
async function signup(event) {
  ...
  try {
    ...
    const orgRef = await db.collection('organizations').add({ name: orgName, email, ownerId: uid, taxRate: TAX_RATE, currency: CURRENCY, country: 'Zambia', themeColor: '#6366f1', createdAt: firebase.firestore.FieldValue.serverTimestamp() });
    await db.collection('users').doc(uid).set({ fullName, email, organizationId: orgRef.id, role: 'admin', createdAt: firebase.firestore.FieldValue.serverTimestamp() });
    msg.className = 'msg success';
...
async function joinWithCode(event) {
  ...
  try {
    ...
    if (inviteSnap.empty) { msg.textContent = 'Invalid or expired invite code'; return; }
    const inviteDoc = inviteSnap.docs[0];
    const invite = inviteDoc.data();
    msg.textContent = 'Creating account...';
    const userCred = await auth.createUserWithEmailAndPassword(invite.email, password);
    await db.collection('users').doc(userCred.user.uid).set({ fullName: invite.fullName, email: invite.email, organizationId: invite.organizationId, role: invite.role, createdAt: firebase.firestore.FieldValue.serverTimestamp() });
    await db.collection('invitations').doc(inviteDoc.id).update({ status: 'accepted', acceptedAt: firebase.firestore.FieldValue.serverTimestamp(), acceptedBy: userCred.user.uid });
    msg.className = 'msg success';
    msg.textContent = `✓ Welcome to ${invite.orgName}!`;
  } catch (err) { msg.textContent = err.code === 'auth/email-already-in-use' ? 'Email already registered. Please sign in.' : err.message; }
}
...
auth.onAuthStateChanged(async (user) => {
  if (user) { currentUser = user; await showPOS(); }
  else { currentUser = null; currentUserData = null; showLogin(); }
});